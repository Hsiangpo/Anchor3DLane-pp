...
def test_openlane(model,
    ...
    use_sigmoid = model.module.test_cfg.get("use_sigmoid", False)   # whether logits are use softmax or sigmoid
    ...
    for batch_indices, data in tqdm.tqdm(zip(loader_indices, data_loader)):
        with torch.no_grad():
            ...
            for output in outputs['proposals_list']:
                result = postprocess(output, anchor_len=dataset.anchor_len, use_sigmoid=use_sigmoid)
                results.append(result)
    dataset.format_results(results, pred_file, use_sigmoid=use_sigmoid)
    ...
    if eval_splits:
        print("evaluating split results...")
        test_lists = ['1000_curve.txt', '1000_intersection.txt', '1000_night.txt', '1000_extreme_weather.txt', '1000_merge_split_case.txt', '1000_updown.txt']
        for test_list in test_lists:
            test_result, single_results = dataset.eval(pred_file, prob_th=prob_th, test_list=test_list)
            print("===> Evaluation on validation set: \n"   
                    "laneline F-measure {:.4} \n"
                    "laneline Recall  {:.4} \n"
                    "laneline Precision  {:.4} \n"
                    ...
                    "laneline z error (far)  {:.4} m\n".format(test_result['F_score'], test_result['recall'],
                                                                ...
                                                                test_result['z_error_close'], test_result['z_error_far']))
            print("save test result to", osp.join(out_dir, 'evaluation_result.json'))
            with open(osp.join(out_dir, f'{test_list[:-4]}.json'), 'w') as f:
                json.dump(test_result, f)
